...
class Game:
    ...
    def key_handler(self, event):
        ...
        if key_value in gui.UP_KEYS:
            self.up()
        elif key_value in gui.LEFT_KEYS:
            self.left()
        elif key_value in gui.DOWN_KEYS:
            self.down()
        elif key_value in gui.RIGHT_KEYS:
            self.right()
        else:
            pass
    ...
    def you_win(self):
        if not self.won:
            self.won = True
            print('You Win!')
            if messagebox.askyesno('2048', 'You beat 2048!\n''continue?'):
                self.keep_playing = True

    def game_over(self):
        print('Game over!')
        messagebox.showinfo('2048', 'well that suck\n''better luck next time')
        self.keep_playing = True
